...
class Encoder:
    ...
    def __init__(self, n_classes, klasses, colors, decode=None):
        ...
        # self.colors = {self.map[i]: np.array(colors[i]) / 255.0 for i in self.map}
        self.color_visualizes = {self.map[i]: np.array(colors[i]) for i in self.map}
        self.class_color_visualizes = np.zeros((n_classes, 3), dtype=np.float32)
        for i in range(n_classes):
            self.class_color_visualizes[i] = self.color_visualizes[i]
            
        # self.class_color_embeddings = np.array([[ 63.75,  63.75,  63.75],
        #     [ 63.75,  63.75, 191.25],
        #     [ 63.75, 191.25,  63.75],
        #     [ 63.75, 191.25, 191.25],
        #     [191.25,  63.75,  63.75],
        ...
        self.class_color_embeddings = self.class_color_visualizes
    ...
    def segmap2color(self, dense):
        ...
        for class_id in np.unique(dense):
            if class_id == Encoder.ignore_index:
                continue
            # rgb[dense == class_id] = self.colors[class_id]
            rgb[dense == class_id] = self.class_color_embeddings[class_id]
    ...
    def encode_segmap(self, x):
        """
            ...
            datasets provide the semantic GT as a RGB map only...
        """
        if x.shape[-1] == 3: # RGB
