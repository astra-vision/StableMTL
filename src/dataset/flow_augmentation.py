...
def flip_optical_flow(flow, flip_mode, valid_mask=None):
    assert flip_mode in ['lr', 'ud']
    if flip_mode == 'lr':
...
def random_flip(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, flip_mode):
    ...
    # flip optical_flow and scene_flow
...
def crop_image_with_optical_flow(image1, image2, flow_2d, flow_2d_valid_mask, crop_window):
    ...
    flow_2d_valid_mask = flow_2d_valid_mask[y1:y2, x1:x2].copy()



    return image1, image2, flow_2d, flow_2d_valid_mask
...
def resize_sparse_flow_map(flow, valid_mask, target_w, target_h):
    ...
    valid_mask_resized = np.zeros([target_h, target_w], dtype=bool)
    valid_mask_resized[yy, xx] = 1.0
    return flow_resized, valid_mask_resized[..., None]
...
def random_scale(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, scale_range):
    ...
    # resize images and optical_flow
...
def random_crop_optical_flow(image1, image2, flow_2d, flow_2d_valid_mask, crop_size):
    assert len(crop_size) == 2
    crop_w, crop_h = crop_size
    ...
    return crop_image_with_optical_flow(image1, image2, flow_2d, flow_2d_valid_mask, crop_window)



def random_flip_optical_flow(image1, image2, flow_2d, flow_2d_valid_mask, flip_mode):
    assert flip_mode in ['lr', 'ud']
    ...
    # flip optical_flow and scene_flow
    new_flow_2d, new_flow_2d_valid_mask = flip_optical_flow(flow_2d, flip_mode, valid_mask=flow_2d_valid_mask)

    return new_image1, new_image2, new_flow_2d, new_flow_2d_valid_mask

def random_flip_flows(image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask, flip_mode):
    assert flip_mode in ['lr', 'ud']

    if np.random.rand() < 0.5:  # do nothing
        return image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask
    ...
    # flip optical_flow and scene_flow
    new_flow_2d, new_flow_2d_valid_mask = flip_optical_flow(flow_2d, flip_mode, valid_mask=flow_2d_valid_mask)
    new_flow_3d, new_flow_3d_valid_mask = flip_scene_flow_no_pc(flow_3d, flow_3d_valid_mask, flip_mode)

    return new_image1, new_image2, new_flow_2d, new_flow_2d_valid_mask, new_flow_3d, new_flow_3d_valid_mask

def joint_flows_augmentation(image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask, cfgs):
    if not cfgs.enabled:
        return image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask
    ...
    if cfgs.random_horizontal_flip.enabled:
        image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask = random_flip_flows(
            image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask, flip_mode='lr'
        )

    if cfgs.random_vertical_flip.enabled:
        image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask = random_flip_flows(
            image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask, flip_mode='ud'
        )

    return image1, image2, flow_2d, flow_2d_valid_mask, flow_3d, flow_3d_valid_mask

def crop_image_optical_flow(image1, image2, flow_2d, flow_2d_valid_mask, crop_window, drop_pc=False):
    assert len(crop_window) == 4  # [x1, y1, x2, y2]
...
def random_scale_optical_flow(image1, image2, flow_2d, flow_2d_valid_mask, scale_range):
    assert len(scale_range) == 2
    assert 1 <= scale_range[0] < scale_range[1]
    ...
    image1, image2, flow_2d, flow_2d_valid_mask = crop_image_optical_flow(
        image1, image2, flow_2d, flow_2d_valid_mask, crop_window
    )

    # resize images and optical_flow
    image1 = cv2.resize(image1, (image_w, image_h), interpolation=cv2.INTER_LINEAR)
    image2 = cv2.resize(image2, (image_w, image_h), interpolation=cv2.INTER_LINEAR)
    flow_2d, flow_2d_valid_mask = resize_sparse_flow_map(flow_2d, flow_2d_valid_mask, image_w, image_h)
...
def joint_optical_flow_augmentation(image1, image2, flow_2d, flow_2d_valid_mask, cfgs):
    if not cfgs.enabled:
        return image1, image2,  flow_2d, flow_2d_valid_mask
    ...
    if cfgs.random_horizontal_flip.enabled:
        image1, image2, flow_2d, flow_2d_valid_mask = random_flip_optical_flow(
            image1, image2, flow_2d, flow_2d_valid_mask, flip_mode='lr'
        )

    if cfgs.random_vertical_flip.enabled:
        image1, image2, flow_2d, flow_2d_valid_mask = random_flip_optical_flow(
            image1, image2, flow_2d, flow_2d_valid_mask, flip_mode='ud'
        )

    if cfgs.random_crop.enabled:
        image1, image2, flow_2d, flow_2d_valid_mask = random_crop_optical_flow(
            image1, image2, flow_2d, flow_2d_valid_mask,
            crop_size=cfgs.random_crop.crop_size,
        )

    if cfgs.random_scale.enabled:
        image1, image2, flow_2d, flow_2d_valid_mask = random_scale_optical_flow(
            image1, image2, flow_2d, flow_2d_valid_mask,
            scale_range=cfgs.random_scale.scale_range
        )
...
def flip_scene_flow_no_pc(flow_3d, flow_3d_valid_mask, flip_mode):
    assert flip_mode in ['lr', 'ud']
    if flip_mode == 'lr':
        flow_3d = np.fliplr(flow_3d).copy()
        flow_3d[:, :, 0] = -flow_3d[:, :, 0]
    elif flip_mode == 'ud':
        flow_3d = np.flipud(flow_3d).copy()
        flow_3d[:, :, 1] = -flow_3d[:, :, 1]
    return flow_3d, flow_3d_valid_mask

def random_flip_scene_flow(image1, image2, flow_3d, flow_3d_valid_mask, flip_mode):
    assert flip_mode in ['lr', 'ud']

    if np.random.rand() < 0.5:  # do nothing
        return image1, image2, flow_3d, flow_3d_valid_mask
    ...
    return new_image1, new_image2, new_flow_3d, new_flow_3d_valid_mask


def joint_scene_flow_augmentation(image1, image2, flow_3d, flow_3d_valid_mask, cfgs):
    if not cfgs.enabled:
        return image1, image2,  flow_3d, flow_3d_valid_mask
    ...
    if hasattr(cfgs, 'random_horizontal_flip') and cfgs.random_horizontal_flip.enabled:
        image1, image2, flow_3d, flow_3d_valid_mask = random_flip_scene_flow(
            image1, image2, flow_3d, flow_3d_valid_mask, flip_mode='lr'
        )

    if hasattr(cfgs, 'random_vertical_flip') and cfgs.random_vertical_flip.enabled:
        image1, image2, flow_3d, flow_3d_valid_mask = random_flip_scene_flow(
            image1, image2, flow_3d, flow_3d_valid_mask, flip_mode='ud'
        )
