...
def eval_dic_to_text(val_metrics: dict, dataset_name: str, sample_list_path: str):
    ...
    # Format all metric values to 6 decimal places before tabulating
    formatted_vals = [
        f"{v:.6f}" if isinstance(v, float) else str(v)
        for v in val_metrics.values()
    ]
    eval_text += tabulate([list(val_metrics.keys()), formatted_vals])
    
    eval_text += "\n"
    # Append CSV representation: first line keys, second line values
    csv_keys = ",".join(val_metrics.keys())
    csv_vals = ",".join(str(v) for v in val_metrics.values())
    eval_text += f"{csv_keys}\n{csv_vals}\n"
    return eval_text